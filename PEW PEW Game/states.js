...
function mainGame(e) {
    ...
    for (let i = objects.length - 1; i >= 0; i--) {
        if (objects[i] instanceof Enemy) {
            if (e.target.getTicks() % 4 == 0 && !lessLag)
    ...
    scoreLabel.font = "bold " + 22 * (Math.sqrt(45 / scoreLabel.getBounds().width)) * hScl + "px Arial";
...
function setupShop() {
    ...
    cashLabel.text = "฿" + cash;
    cashLabel.font = "bold " + 24 * (Math.sqrt(100 / cashLabel.getBounds().width)) * hScl + "px Arial";
...
function updateGun(newIndex) {
    ...
    gunImg.x = (-1 * guns[gunIndex].image.x + 87) * hScl;
    gunImg.y = (-1 * guns[gunIndex].image.y + 33) * hScl;
    gunImg.scaleX = hScl;
    gunImg.scaleY = hScl;
